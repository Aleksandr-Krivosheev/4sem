...
void task(multigraph g)
{
	...
	while (available_nodes.size() != 0)
	{
		...
		// Подсчет количества связей с назначенными вершинами для первой свободной вершины
		for (int j = 0; j < g.nodes; j++)
		{
			if (find(available_nodes.begin(), available_nodes.end(), j + 1) == available_nodes.end())
				max_appointed += g.graph[available_nodes[0] - 1][j];
		}

		// Вывод количества связей для первой вершины
		cout << node << " " << max_appointed << endl;

		// Подсчет количества связей с назначенными  вершинами для каждой свободной вершины (начиная со второй по счету)
		for (int i = 1; i < available_nodes.size(); i++)
		{
			...
			{
				if (find(available_nodes.begin(), available_nodes.end(), j + 1) == available_nodes.end())
					appointed += g.graph[available_nodes[i] - 1][j];
			}

			// Поиск максимума
			if (appointed > max_appointed)
			{
				node = available_nodes[i];
				max_appointed = appointed;
			}

			// Вывод количества связей для текущей вершины
			cout << available_nodes[i] << " " << appointed << endl;
		}

		// Вывод найденной вершины, которая имеет максимальное количество связей с назначенными
		cout << node << " " << max_appointed << endl;
