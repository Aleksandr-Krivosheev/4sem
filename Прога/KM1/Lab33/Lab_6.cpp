...
void solution()
{
    ...
    for (int a = 0; a < n; a++)
    {
        ...
        if (count == planes.getsize())
            cout << "В эропорту " << air[a].getname() << " на эксплуататции все типы самолётов" << endl << endl;
    }
    ...
    for (int i = 0; i < planes.getsize(); i++)
    {
        ...
        if (count == 0)
        {
            cout << planes.getm(i) << " не эксплуатируется ни в одном из аэропортов" << endl << endl;
        }
    }
